...
export default function JokesReview() {
  ...
  const handleBulkUpdate = async (status: 'APPROVED' | 'REJECTED') => {
    ...
    const result = await Swal.fire({
      ...
      text: `You are about to ${actionText.toLowerCase()} ${selectedJokes.length} selected jokes.`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: actionColor,
      cancelButtonColor: '#6b7280',
      confirmButtonText: `Yes, ${actionText.toLowerCase()} all!`
    });

    if (result.isConfirmed) {
      try {
        await Promise.all(selectedJokes.map(id => apiClient.put(`/jokes/${id}/status`, { status })));
        ...
        Swal.fire({
          icon: 'success',
          title: `${actionText}d!`,
          text: `The selected jokes have been ${status.toLowerCase()}.`,
          showConfirmButton: false,
          timer: 1500
        });
      } catch (error) {
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: `Failed to process some or all selected jokes.`
        });
      }
    }
  };
  ...
  const toggleSelectAll = () => {
    if (selectedJokes.length === filteredJokes.length) {
      ...
      setSelectedJokes(filteredJokes.map(joke => joke.id));
    }
  };
        ...
        <table className="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700 text-sm text-left text-zinc-600 dark:text-zinc-400">
          ...
          <tbody className="divide-y divide-zinc-200 dark:divide-zinc-700">
            {loading ? (
              ...
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center">No jokes pending review.</td>
              </tr>
            )}
          </tbody>
        </table>
